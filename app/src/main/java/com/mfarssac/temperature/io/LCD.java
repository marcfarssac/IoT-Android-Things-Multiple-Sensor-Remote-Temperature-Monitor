package com.mfarssac.temperature.io;
...
//        sendCommand(LCD_DISPLAY_ON);
    ...
    public void writeTime() throws IOException {
        ...
        if (timeLoop%2 == 1) {separator = " "; timeLoop =1;} else {timeLoop=0;}
        int hh = (int)(System.currentTimeMillis()/1000/3600)%12;
        int mm = (int)(System.currentTimeMillis()/1000/60)%60;

        String time = String.format(Locale.GERMAN, "%02d",hh) + separator + String.format(Locale.GERMANY,"%02d",mm);
        writeText(text1, time);
        timeLoop++;
